...
The HTTP contract is stable regardless of backend internals. Strategic selector is **`ERP_EXECUTION_BACKEND`** with `docker|remote`: `docker` is temporary compatibility (default), and `remote` is the long-term target scaffold. Legacy `host_bench` notes may still exist for internal operations context, but `host_bench` is not part of the approved long-term architecture path. See `provisioning-agent/docs/erp-execution-backend.md` and `provisioning-agent/docs/erp-side-execution-interface.md`.
